...
namespace FirstCommand
{
    ...
    public class RailingAnalyzer : IExternalCommand
    {
        ...
        private void PrepareDataForExecution(List<CylinderInfo> cylinderInfos, Document doc, UIDocument uiDoc, List<Mesh> meshes)
        {
            ...
            if (GroupPointsOnSamePlane(mergedList).Count == 2)
            {
                ...
                if (arePointsColinear)
                {
                    ...
                    GetTopViewShape(meshes, mergedList);
                }
                else
                {
                    GetTopViewShape(meshes, mergedList);
        ...
        private void GetTopViewShape(List<Mesh> meshes, List<CylinderInfo> cylinderInfos)
        {
            if (cylinderInfos.Count > 2)
            {
                ...
                GetTrianglesHaveHigherZ(trianglesOfElement, radius);
                ...
                //var tupleValues = GetLinesFromTriangles(trianglesPresentForTopViewShape, firstAndLastPoint, radius, true);
                ...
                //    GetAllIntersecPoints(firstPoint, lastPoint, firstLine, lastLine, lines);
                //}
        ...
        private List<MeshTriangle> GetTrianglesHaveHigherZ(List<MeshTriangle> triangles, double r)
        {
            ...
            foreach (var keyValue in keyValuePairs)
            {
                ...
                for (int i = 0; i < trianglesOnGrid.Count; i++)
                {
                    for (int j = trianglesOnGrid.Count - 1; j > i; j--)
                    {
                        if (IsTrianglesIntersect(trianglesOnGrid[i], trianglesOnGrid[j]))
                        {
                            MeshTriangle triangleLower = GetTriangleLower(trianglesOnGrid[i], trianglesOnGrid[j]);
                            trianglesOnGrid.Remove(triangleLower);
                        }
                    }
                }
            }
        ...
        /// <returns></returns>
        private MeshTriangle GetTriangleLower(MeshTriangle tri1, MeshTriangle tri2)
        {
        ...
        private (Line FirstLine, Line LastLine, List<Line> Lines) GetLinesFromTriangles(List<MeshTriangle> triangles, List<XYZ> points, double radius, bool isTopPointOfCylinder)
        {
            List<Line> lines = new List<Line>();
            Line firstLine = null;
            Line lastLine = null;
            ...
            List<XYZ> targetPoints = GetRemainTriangles(pointAndLines, triangles, radius);
            if (triangles.Count > 0)
            {
                if (targetPoints.Count > 0)
                {
                    List<XYZ> newPoints = new List<XYZ> { targetPoints.First() };
                    lines.AddRange(GetLinesFromTriangles(triangles, newPoints, radius, false).Lines);
                }
            }
        ...
        /// <param name="r"></param>
        /// <returns></returns>
        private List<XYZ> GetRemainTriangles(List<(XYZ, Line)> pointAndLines, List<MeshTriangle> triangles, double r)
        {
            ...
            foreach (var tuple in pointAndLines)
            {
                ...
                XYZ farthestPoint = GetFarthestPoint(tuple.Item1, trianglesNearLine);
                targetPoints.Add(farthestPoint);
                trianglesViewed.AddRange(trianglesNearLine);
            }
        ...
        private List<MeshTriangle> GetSublistTriangles(List<MeshTriangle> triangles, XYZ point, double length)
        {
        ...
        private XYZ GetAxisFromTriangles(List<MeshTriangle> triangles)
        {
            ...
            // vì là hình chiếu từ trên xuống nên Z = 0
