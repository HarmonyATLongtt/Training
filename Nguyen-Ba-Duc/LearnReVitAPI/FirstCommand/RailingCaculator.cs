...
namespace FirstCommand
{
    ...
    public class RailingCaculator : IExternalCommand
    {
        ...
        /// <summary>
        /// Chuẩn bị dữ liệu để thực thi cho việc tính toán chiều dài lan can
        /// </summary>
        ...
        private void PrepareDataForExecution(List<CylinderInfo> cylinderInfos, Document doc, UIDocument uiDoc)
        {
            ...
            var sortedList = SortInstancesAlongLine(mergedList);
            ...
            if (GroupPointsOnSamePlane(sortedList).Count == 0)
            {
                var results = new List<List<CylinderInfo>> { sortedList };

                CalculateSpiralRailingLength(results, doc, uiDoc);
            }
            else
            {
                var result = GroupPointsForDistanceCalculation(sortedList);
                double length = 0;
            ...
            if (solids.Count > 0)
            {
                foreach (Solid solid in solids)
                {
                    ...
                    if (planarFaces.Count > 0 && cylindricalFaces.Count > 0)
                    {
                        ...
                        var cylindricalFacesParallelToZ = cylindricalFaces.Where(c => Math.Abs(Math.Abs(c.Axis.Z) - 1) < tolerance).ToList();
                        var horizontalPlanarFaces = planarFaces.Where(f => Math.Abs(Math.Abs(f.FaceNormal.Z) - 1) < tolerance).ToList();
                        if (cylindricalFacesParallelToZ.Count > 0)
                        {
                            if (horizontalPlanarFaces.Count == 2)
                            {
                                CylindricalFace cylindricalFace = cylindricalFacesParallelToZ.FirstOrDefault();
                                cylinderInfos.Add(CreateCylinderInfo(cylindricalFace, horizontalPlanarFaces.Max(f => f.Origin.Z), horizontalPlanarFaces.Min(f => f.Origin.Z)));
                                //cylinderInfos.Add(CreateCylinderInfo(cylindricalFace, planarFaces.Max(f => f.Origin.Z), planarFaces.Min(f => f.Origin.Z)));
                            }
                            else
                            {
                                // Lọc ra tất cả các CylindricalFace đại diện cho 1 cột trụ, 1 cột trụ có thể có nhiều
                                // CylindricalFace cùng chung axis và origin
                                XYZ firstOrigin = new XYZ();
                                var newListFace = new List<CylindricalFace>();
                                for (int i = 0; i < cylindricalFacesParallelToZ.Count; i++)
                                {
                                    if (!cylindricalFacesParallelToZ[i].Origin.IsAlmostEqualTo(firstOrigin, tolerance))
                                    {
                                        newListFace.Add(cylindricalFacesParallelToZ[i]);
                                        firstOrigin = cylindricalFacesParallelToZ[i].Origin;
                                    }
                                }
        ...
        private List<List<CylinderInfo>> GroupPointsForDistanceCalculation(List<CylinderInfo> cylinderInfos)
        {
            ...
            foreach (var group in result)
            {
                var sortedGroup = SortInstancesAlongLine(group);
                var newGroup = CloneList(sortedGroup);
