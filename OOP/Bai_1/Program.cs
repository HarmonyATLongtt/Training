...
class Program
{
    ...
    static void Init()
    {
        ...
        FileInfo existingFile = new FileInfo(@"F:\Coding\c#\Excel_data\Bai1.xlsx");
        using (ExcelPackage package = new ExcelPackage(existingFile))
        {
            ...
            for (int row = 2; row <= rowCount; row++)
            {
                ...
                string tax_coe = worksheet.Cells[row, 5].Value.ToString().Trim();
                person myperson = new person(id,name,age,income,tax_coe);
